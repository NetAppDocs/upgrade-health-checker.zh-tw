...
NOTE: 請確保虛擬機器可以透過 HTTPS（443）連接到自動更新和遙測端點(https://support.netapp.com/[]和 https://api.activeiq.netapp.com[]，以便 NetApp 接收有關升級計劃的 AutoSupport 資訊。如果無法存取網際網路，則必須手動下載最新版本的 Upgrade Health Checker。

.步驟
. 瀏覽至link:https://console.netapp.com/automationHub["NetApp Console 自動化中心"^]並找到 Upgrade Health Checker 磚，以下載 Upgrade Health Checker 二進位檔。
...
.相關資訊
* link:upgrade-health-checker-input-parameters.html["了解 Upgrade Health Checker 的輸入參數"]
